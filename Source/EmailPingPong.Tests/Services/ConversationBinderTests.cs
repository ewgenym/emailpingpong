...
namespace EmailPingPong.Tests.Services
{
	public class ConversationBinderTests
	{
		...
		public ConversationBinderTests()
		{
			...
			_conversationBinder = new ConversationBinder(_conversationParser, _emailItemBinder, _metadataTracker, _timeProvider);
		...
		[Fact]
		public void should_bind_conversation_creation_date_time()
		{
			var createdOn = new DateTime(2013, 2, 16, 16, 18, 55);
			_timeProvider.Now.Returns(createdOn);

			// act
			var conversation = _conversationBinder.Bind(_mailItem);

			// assert
			conversation.CreatedOn.Should().Be(createdOn);
		}
	}
}